...
import type { CERTIFICATIONS_QUERYResult } from "@/sanity.types";

const CERTIFICATIONS_QUERY =
  defineQuery(`*[_type == "certification"] | order(featuredCertification desc, order asc, issueDate desc){
  name,
  issuer,
  issueDate,
  expiryDate,
...
type Certification = CERTIFICATIONS_QUERYResult[number];
type CertificationSkill = NonNullable<
  NonNullable<Certification["skills"]>[number]
>;
...
export async function CertificationsSection() {
  ...
  const certifications = (data ?? []).filter(
    (cert): cert is Certification => Boolean(cert?.featuredCertification),
  );
        ...
        <div className="@container">
          <div className="grid grid-cols-1 @2xl:grid-cols-2 gap-10">
            {certifications.map((cert: Certification) => {
              const certName =
                typeof cert.name === "string" ? cert.name : "Certification";
              const certIssuer =
                typeof cert.issuer === "string" ? cert.issuer : "Issuer";

              return (
                <CometCard
                  key={`${certIssuer}-${certName}-${cert.issueDate}`}
                  rotateDepth={8}
                  translateDepth={10}
                  className="w-full"
                >
